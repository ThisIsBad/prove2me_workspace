...
theorem solution (p : ℕ → ℝ) (f : ℕ → ℕ → ℝ) (hf : ∀ j, IsPmf (f j))
    ...
    (∀ j x d, x ≤ C → IsStageOptimal p f j x d (bidControl p f j x d)) := by
  classical
  have hc := static_conc p f hf
  have hmem : ∀ j k, 1 ≤ k → k ≤ C → p (j + 1) < staticDelta p f j k → k ≤ protLevel p f C j := by
    intro j k hk hkC hlt
    unfold protLevel
    exact Finset.le_sup (f := id) (Finset.mem_filter.2 ⟨Finset.mem_range.2 (by omega), hk, hlt⟩)
  have hin : ∀ j, 1 ≤ protLevel p f C j →
      p (j + 1) < staticDelta p f j (protLevel p f C j) := by
    intro j h
    unfold protLevel at h ⊢
    have := rm_sup_mem h
    exact (Finset.mem_filter.1 this).2.2
  have hle : ∀ j, protLevel p f C j ≤ C := by
    intro j
    unfold protLevel
    apply Finset.sup_le
    intro b hb
    have := (Finset.mem_filter.1 hb).1
    simp only [Finset.mem_range] at this
    simp only [id]; omega
  have h2 : ∀ j x d, x ≤ C → IsStageOptimal p f j x d (min (x - protLevel p f C j) d) := by
    intro j x d hxC
    refine ⟨by omega, rm_stage_opt (g := staticValue p f j) (p (j + 1)) d x _ (by omega) ?_ ?_⟩
    · intro v hv
      by_contra H
      push Not at H
      have := hmem j (x - v) (by omega) (by omega) H
      omega
    · intro v hv1 hv2
      have hy : x - v ≤ protLevel p f C j := by omega
      have hy1 : 1 ≤ protLevel p f C j := by omega
      have h1 := hin j hy1
      have h3 := rm_conc_mono (hc j) (a := x - v) (b := protLevel p f C j) (by omega) hy
      unfold staticDelta at h1
      linarith
  refine ⟨?_, h2, ?_, ?_⟩
  · intro j
    unfold protLevel
    ...
    have h1 : p (j + 1 + 1) ≤ p (j + 1) := hanti (Nat.le_succ _)
    have h3 := static_mono p f hf j k hk.2.1
    linarith [hk.2.2]
  · intro j x d hxC
    have e : bookLimit p f C (j + 1) - (C - x) = x - protLevel p f C j := by
      unfold bookLimit
      simp only [Nat.add_sub_cancel]
      have := hle j
      omega
    ...
    exact h2 j x d hxC
  · intro j x d _
    set s := (Finset.range (min d x + 1)).filter
      (fun z => z = 0 ∨ bidPrice p f (j + 1) (x + 1 - z) ≤ p (j + 1)) with hs
    have hbid : bidControl p f j x d = s.sup id := rfl
    have hbp : ∀ k, bidPrice p f (j + 1) k = staticValue p f j k - staticValue p f j (k - 1) := by
      intro k; unfold bidPrice staticDelta; simp only [Nat.add_sub_cancel]
    have hne : s.Nonempty := ⟨0, Finset.mem_filter.2 ⟨Finset.mem_range.2 (by omega), Or.inl rfl⟩⟩
    obtain ⟨z, hz, hze⟩ := Finset.exists_mem_eq_sup s hne id
    rw [hbid, hze]
    have hz' := Finset.mem_filter.1 hz
    have hzle : z ≤ min d x := by have := Finset.mem_range.1 hz'.1; omega
    refine ⟨hzle, rm_stage_opt (g := staticValue p f j) (p (j + 1)) d x z hzle ?_ ?_⟩
    · intro v hv
      rcases hz'.2 with h0 | hb
      · omega
      · rw [hbp] at hb
        have h3 := rm_conc_mono (hc j) (a := x + 1 - z) (b := x - v) (by omega) (by omega)
        linarith
    · intro v hv1 hv2
      by_contra H
      push Not at H
      have hmemv : v + 1 ∈ s := by
        ...
        refine ⟨Finset.mem_range.2 (by omega), Or.inr ?_⟩
        rw [hbp, show x + 1 - (v + 1) = x - v by omega]
        linarith
      have := Finset.le_sup (f := id) hmemv
      rw [hze] at this
