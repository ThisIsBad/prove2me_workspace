...
theorem solution (D : DiscreteDemand) (h b1 A μ : ℝ) (hh : 0 < h) (hb : 0 < b1)
    ...
    ∀ (R : ℤ) (Q : ℕ), 1 ≤ Q →
      rqDiscreteCost D h b1 A μ Rstar Qstar ≤ rqDiscreteCost D h b1 A μ R Q := by
  have hpos : 0 ≤ h + b1 := by linarith
  have hopt : ∀ Q : ℕ, 1 ≤ Q → ∃ R : ℤ, rqDiscreteCost D h b1 A μ R Q = CQ Q ∧
      ∀ R' : ℤ, CQ Q ≤ rqDiscreteCost D h b1 A μ R' Q := by
    intro Q hQ
    obtain ⟨R, hR⟩ := (hCQ Q hQ).1
    exact ⟨R, hR, fun R' => (hCQ Q hQ).2 ⟨R', rfl⟩⟩
  choose! Rq hRq using hopt
  have hWopt : ∀ Q : ℕ, 1 ≤ Q → ∀ R : ℤ, ic5W D h b1 (Rq Q) Q ≤ ic5W D h b1 R Q := by
    intro Q hQ R
    rw [← ic5_W_le_iff D h b1 A μ _ _ Q hQ, (hRq Q hQ).1]
    exact (hRq Q hQ).2 R
  -- (Q+1) C(Q+1) = Q C(Q) + m(Q)
  have hrec : ∀ Q : ℕ, 1 ≤ Q →
      CQ (Q + 1) * ((Q : ℝ) + 1) = CQ Q * Q + ic5m D h b1 Q (Rq Q) := by
    intro Q hQ
    have hv := ic5_W_value D h b1 hpos Q hQ (Rq Q) (Rq (Q + 1)) (hWopt Q hQ) (hWopt (Q + 1) (by omega))
    have e1 := ic5_cost_eq D h b1 A μ (Rq Q) Q hQ
    have e2 := ic5_cost_eq D h b1 A μ (Rq (Q + 1)) (Q + 1) (by omega)
    rw [(hRq Q hQ).1] at e1
    rw [(hRq (Q + 1) (by omega)).1] at e2
    have hQ' : (Q : ℝ) ≠ 0 := by have : (0:ℝ) < Q := by exact_mod_cast hQ
                                 positivity
    rw [e1, e2, hv]; push_cast
    field_simp
    ring
  -- the marginal m(Q) is nondecreasing
  have hmono : ∀ Q : ℕ, 1 ≤ Q → ic5m D h b1 Q (Rq Q) ≤ ic5m D h b1 (Q + 1) (Rq (Q + 1)) := by
    intro Q hQ
    have h1 := ic5_m_step D h b1 hpos Q hQ (Rq Q) (hWopt Q hQ)
    have h2 := ic5_m_le D h b1 hpos (Q + 1) (by omega) (ic5next D h b1 Q (Rq Q)) (Rq (Q + 1))
      (ic5_next_opt D h b1 hpos Q hQ (Rq Q) (hWopt Q hQ)) (hWopt (Q + 1) (by omega))
    exact h1.trans h2
  -- from Qstar on, the costs do not decrease
  have hup : ∀ t : ℕ, CQ (Qstar + t) ≤ ic5m D h b1 (Qstar + t) (Rq (Qstar + t))
      ∧ CQ Qstar ≤ CQ (Qstar + t) := by
    intro t
    induction t with
    | zero =>
      refine ⟨?_, le_refl _⟩
      have := hrec Qstar hQ1
      have hq : (0 : ℝ) ≤ (Qstar : ℝ) + 1 := by positivity
      have := mul_le_mul_of_nonneg_right hstop hq
      simp only [add_zero]
      nlinarith
    | succ n ih =>
      obtain ⟨ih1, ih2⟩ := ih
      have hr := hrec (Qstar + n) (by omega)
      have hm := hmono (Qstar + n) (by omega)
      have hq : (0 : ℝ) < ((Qstar + n : ℕ) : ℝ) + 1 := by positivity
      have e : Qstar + (n + 1) = Qstar + n + 1 := by ring
      rw [e]
      constructor
      · by_contra hc; push Not at hc
        have : CQ (Qstar + n + 1) > ic5m D h b1 (Qstar + n) (Rq (Qstar + n)) := by linarith
        nlinarith
      · by_contra hc; push Not at hc
        nlinarith
  -- before Qstar, the costs strictly decrease
  have hdown : ∀ d : ℕ, ∀ Q : ℕ, 1 ≤ Q → Q + d = Qstar → CQ Qstar ≤ CQ Q := by
    intro d
    induction d with
    | zero => intro Q _ hQ; rw [← hQ]; simp
    | succ n ih =>
      intro Q hQ hQd
      have h1 := ih (Q + 1) (by omega) (by omega)
      have h2 := hfirst Q hQ (by omega)
      linarith
  have hCstar : rqDiscreteCost D h b1 A μ Rstar Qstar = CQ Qstar :=
    le_antisymm (by rw [← (hRq Qstar hQ1).1]; exact hR _) ((hRq Qstar hQ1).2 Rstar)
  intro R Q hQ
  rw [hCstar]
  refine le_trans ?_ ((hRq Q hQ).2 R)
  rcases le_or_gt Qstar Q with hle | hlt
  · have := (hup (Q - Qstar)).2
    rwa [show Qstar + (Q - Qstar) = Q by omega] at this
  · exact hdown (Qstar - Q) Q hQ (by omega)
