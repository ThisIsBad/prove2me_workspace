...
theorem solution (lam : ℕ → ℕ → ℝ) (p : ℕ → ℝ) (n T : ℕ)
    (hlam : IsArrivalModel lam n) (hp : ∀ j, 0 ≤ p j) (t x : ℕ) (ht : 1 ≤ t) (htT : t ≤ T)
    (hx : 1 ≤ x) :
    dynDelta lam p n T t (x + 1) ≤ dynDelta lam p n T t x ∧
      dynDelta lam p n T (t + 1) x ≤ dynDelta lam p n T t x := by
  unfold dynDelta dynValue
  constructor
  · simp only [Nat.add_sub_cancel]
    exact dyn_conc lam p n T hlam _ x hx
  · rw [show T + 1 - t = (T + 1 - (t + 1)) + 1 by omega]
    exact (dyn_step lam p n T hlam _ (dyn_conc lam p n T hlam _)).2 x hx
