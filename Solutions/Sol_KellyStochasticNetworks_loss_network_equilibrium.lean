...
theorem solution {J R : ℕ} (A : Fin J → Fin R → ℕ) (C : Fin J → ℕ)
    ...
    DetailedBalance (fun n : lossStates A C => G * lossWeight ν (n : Fin R → ℕ))
        (truncatedRates
          ...
          (lossStates A C))
      ∧ HasSum (fun n : lossStates A C => G * lossWeight ν (n : Fin R → ℕ)) 1 := by
  have hDB := sn3_lossWeight_detailed ν
  refine ⟨fun j k => ?_, ?_⟩
  · simp only [truncatedRates]
    have := hDB (j : Fin R → ℕ) (k : Fin R → ℕ)
    simp only at this
    rw [mul_assoc, mul_assoc, this]
  · have := hG.mul_left G
    rwa [mul_inv_cancel₀ hG0] at this
