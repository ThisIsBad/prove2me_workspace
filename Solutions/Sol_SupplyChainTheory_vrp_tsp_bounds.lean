...
import Theorems.Thm_SupplyChainTheory_vrp_radial_bound
import Theorems.Thm_SupplyChainTheory_vrp_tsp_le
import Theorems.Thm_SupplyChainTheory_iotp_bound
...
theorem solution {n : ℕ} (c : Fin (n + 1) → Fin (n + 1) → ℝ) (hc : VRPMetric c)
    ...
    max (2 * ((n : ℝ) / C) * avgDepotDist c) (tspOpt c) ≤ vrpOpt c C
      ∧ vrpOpt c C ≤ 2 * ⌈(n : ℝ) / C⌉₊ * avgDepotDist c + (1 - 1 / (C : ℝ)) * tspOpt c := by
  -- Lower bound: the radial bound and the routing bound.
  refine ⟨max_le (vrp_radial_bound c hc hn C hC) (vrp_tsp_le c hc hn C hC), ?_⟩
  -- Upper bound: partition any tour (11.59), then optimize over tours.
  have hCpos : (0 : ℝ) < C := by exact_mod_cast hC
  have hnpos : (0 : ℝ) < n := by exact_mod_cast hn
  set ℓ := ⌈(n : ℝ) / C⌉₊ with hℓ
  have hℓC : (n : ℝ) ≤ ℓ * C := by
    have h := Nat.le_ceil ((n : ℝ) / C); rw [← hℓ, div_le_iff₀ hCpos] at h; exact h
  have hfrac : 1 / (C : ℝ) ≤ (ℓ : ℝ) / n := by
    rw [div_le_div_iff₀ hCpos hnpos]; linarith
  set B := 2 * (ℓ : ℝ) * avgDepotDist c with hB
  have hper : ∀ L, IsCustomerTour L → vrpOpt c C ≤ B + (1 - 1 / (C : ℝ)) * routeCost c L := by
    intro L hL
    obtain ⟨R, hR, hcost⟩ := iotp_bound c hc hn C hC L hL
    have h1 : vrpOpt c C ≤ solutionCost c R := csInf_le (vrp_bddBelow c hc C) ⟨R, hR, rfl⟩
    have h2 : (1 - (ℓ : ℝ) / n) * routeCost c L ≤ (1 - 1 / (C : ℝ)) * routeCost c L :=
      mul_le_mul_of_nonneg_right (by linarith) (routeCost_nonneg c hc L)
    linarith
  rcases eq_or_lt_of_le hC with hC1 | hC2
  · -- `C = 1`: the tour term vanishes.
    subst hC1
    have := hper _ (customers_isTour n)
    simp only [Nat.cast_one, div_one, sub_self, zero_mul, add_zero] at this ⊢
    exact this
  · -- `C ≥ 2`: divide by `1 - 1/C > 0` and take the infimum over tours.
    have hpos : 0 < 1 - 1 / (C : ℝ) := by
      have : (1 : ℝ) < C := by exact_mod_cast hC2
      rw [sub_pos, div_lt_one hCpos]; exact this
    have hinf : (vrpOpt c C - B) / (1 - 1 / (C : ℝ)) ≤ tspOpt c := by
      refine le_csInf ⟨_, customers n, customers_isTour n, rfl⟩ ?_
      rintro z ⟨L, hL, rfl⟩
      rw [div_le_iff₀ hpos]; linarith [hper L hL]
    rw [div_le_iff₀ hpos] at hinf
    linarith
