...
theorem solution (lam mu : ℝ) (hlam : 0 < lam) (hmu : lam < mu) :
    DetailedBalance (fun j : ℕ => (1 - lam / mu) * (lam / mu) ^ j) (mm1Rates lam mu)
      ∧ HasSum (fun j : ℕ => (1 - lam / mu) * (lam / mu) ^ j) 1 := by
  have hmu0 : 0 < mu := by linarith
  have hρ0 : 0 ≤ lam / mu := by positivity
  have hρ1 : lam / mu < 1 := by rw [div_lt_one hmu0]; exact hmu
  have hmu' : mu ≠ 0 := ne_of_gt hmu0
  refine ⟨fun j k => ?_, ?_⟩
  · unfold mm1Rates
    by_cases h1 : k = j + 1
    · subst h1; simp only [if_true, show ¬ (j = j + 1 + 1) by omega, if_false]
      simp only [div_pow, pow_succ]
      field_simp
    · by_cases h2 : j = k + 1
      · subst h2; simp only [if_true, show ¬ (k + 1 = k + 1 + 1) by omega, if_false, h1]
        simp only [div_pow, pow_succ]
        field_simp
      · simp [h1, h2]
  · have := (hasSum_geometric_of_lt_one hρ0 hρ1).mul_left (1 - lam / mu)
    rwa [mul_inv_cancel₀ (by linarith)] at this
