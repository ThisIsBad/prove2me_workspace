...
lemma sn2_indicator_integral (T a d : ℝ) (ha : 0 ≤ a) (had : a ≤ d) (hd : d ≤ T) :
    (∫ s in (0:ℝ)..T, Set.indicator (Set.Ico a d) (fun _ => (1 : ℝ)) s) = d - a := by
  have hT : 0 ≤ T := le_trans (le_trans ha had) hd
  rw [intervalIntegral.integral_of_le hT,
    MeasureTheory.integral_indicator measurableSet_Ico]
  rw [MeasureTheory.Measure.restrict_restrict measurableSet_Ico]
  simp only [MeasureTheory.integral_const, smul_eq_mul, mul_one]
  have h1 : Set.Ioo a d ⊆ Set.Ico a d ∩ Set.Ioc 0 T := by
    intro x hx
    exact ⟨⟨le_of_lt hx.1, hx.2⟩, ⟨lt_of_le_of_lt ha hx.1, le_trans (le_of_lt hx.2) hd⟩⟩
  have h2 : Set.Ico a d ∩ Set.Ioc 0 T ⊆ Set.Icc a d := fun x hx => ⟨hx.1.1, le_of_lt hx.1.2⟩
  have m1 := MeasureTheory.measure_mono (μ := MeasureTheory.volume) h1
  have m2 := MeasureTheory.measure_mono (μ := MeasureTheory.volume) h2
  rw [Real.volume_Ioo] at m1
  rw [Real.volume_Icc] at m2
  have heq : MeasureTheory.volume (Set.Ico a d ∩ Set.Ioc 0 T) = ENNReal.ofReal (d - a) :=
    le_antisymm m2 m1
  simp only [MeasureTheory.Measure.real, MeasureTheory.Measure.restrict_apply MeasurableSet.univ,
    Set.univ_inter, heq]
  rw [ENNReal.toReal_ofReal (by linarith)]

theorem solution {N : ℕ} (T : ℝ) (a d : Fin N → ℝ)
    (ha : ∀ i, 0 ≤ a i) (had : ∀ i, a i ≤ d i) (hd : ∀ i, d i ≤ T) :
    (∫ s in (0:ℝ)..T, occupancy a d s) = ∑ i, (d i - a i) := by
  unfold occupancy
  rw [intervalIntegral.integral_finsetSum]
  · exact Finset.sum_congr rfl (fun i _ => sn2_indicator_integral T (a i) (d i) (ha i) (had i) (hd i))
  · intro i _
