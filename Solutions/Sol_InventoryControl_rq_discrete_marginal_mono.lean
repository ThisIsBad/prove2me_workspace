...
theorem solution (D : DiscreteDemand) (h b1 A μ : ℝ) (hh : 0 < h) (hb : 0 < b1)
    ...
    (rqDiscreteCost D h b1 A μ Rstar Q ≤ rqDiscreteCost D h b1 A μ Rnext (Q + 1)
      ...
      ∧ min (sPolicyCost D h b1 Rstar) (sPolicyCost D h b1 (Rstar + Q + 1))
          ≤ min (sPolicyCost D h b1 Rnext) (sPolicyCost D h b1 (Rnext + (Q + 1) + 1)) := by
  have hpos : 0 ≤ h + b1 := by linarith
  have hQ1 : 0 < Q + 1 := Nat.succ_pos Q
  have hW : ∀ R : ℤ, ic5W D h b1 Rstar Q ≤ ic5W D h b1 R Q :=
    fun R => (ic5_W_le_iff D h b1 A μ Rstar R Q hQ).1 (hR R)
  have hW' : ∀ R : ℤ, ic5W D h b1 Rnext (Q + 1) ≤ ic5W D h b1 R (Q + 1) :=
    fun R => (ic5_W_le_iff D h b1 A μ Rnext R (Q + 1) hQ1).1 (hR' R)
  have hval := ic5_W_value D h b1 hpos Q hQ Rstar Rnext hW hW'
  constructor
  · rw [ic5_cost_eq D h b1 A μ _ (Q + 1) hQ1, ic5_cost_eq D h b1 A μ _ Q hQ, hval]
    unfold ic5m
    have hQ' : (0 : ℝ) < Q := by exact_mod_cast hQ
    have hQ1' : (0 : ℝ) < ((Q + 1 : ℕ) : ℝ) := by exact_mod_cast hQ1
    rw [div_le_div_iff₀ hQ' hQ1', div_le_iff₀ hQ']
    push_cast
    constructor <;> intro H <;> nlinarith
  · have h1 := ic5_m_step D h b1 hpos Q hQ Rstar hW
    have h2 := ic5_m_le D h b1 hpos (Q + 1) hQ1 (ic5next D h b1 Q Rstar) Rnext
      (ic5_next_opt D h b1 hpos Q hQ Rstar hW) hW'
    have h3 := h1.trans h2
    unfold ic5m at h3
    push_cast at h3
    exact h3
