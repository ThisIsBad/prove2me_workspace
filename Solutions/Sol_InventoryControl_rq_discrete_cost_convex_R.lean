...
theorem solution (D : DiscreteDemand) (h b1 A μ : ℝ) (hh : 0 < h) (hb : 0 < b1)
    (Q : ℕ) (hQ : 0 < Q) (R : ℤ) :
    rqDiscreteCost D h b1 A μ (R + 1) Q - rqDiscreteCost D h b1 A μ R Q
      ≤ rqDiscreteCost D h b1 A μ (R + 2) Q - rqDiscreteCost D h b1 A μ (R + 1) Q := by
  have h1 := ic5_cost_diff D h b1 A μ R Q hQ
  have h2 := ic5_cost_diff D h b1 A μ (R + 1) Q hQ
  have e : R + 1 + 1 = R + 2 := by ring
  rw [e] at h2
  have hm := ic5_ready_mono D Q hQ (show R + 1 ≤ R + 2 by omega)
  rw [h1, h2]
  have : 0 < h + b1 := by linarith
  nlinarith
