...
theorem solution {J : ℕ} (lam : Fin J → Fin J → ℝ) (φ : Fin J → ℕ → ℝ)
    ...
    FullBalance (fun n : Fin J → ℕ => G⁻¹ * ∏ j, (α j ^ n j / phiProd φ j (n j)))
      (closedMigrationRates lam φ) := by
  classical
  set u : Fin J → ℕ → ℝ := fun j m => α j ^ m / phiProd φ j m with hudef
  have hu : ∀ j m, u j (m + 1) * φ j (m + 1) = u j m * α j := by
    intro j m
    have := sn2_hu (fun _ => (1 : ℝ)) α φ hφpos j m
    simp only [one_mul] at this
    exact this
  intro n
  beta_reduce
  have hL : HasSum (fun m => closedMigrationRates lam φ n m) (∑ j, ∑ k, lam j k * φ j (n j)) := by
    unfold closedMigrationRates
    exact hasSum_sum (fun j _ => hasSum_sum (fun k _ => hasSum_ite_eq _ _))
  have hpt : ∀ m, (G⁻¹ * ∏ j, (α j ^ m j / phiProd φ j (m j))) * closedMigrationRates lam φ m n
      = ∑ j, ∑ k, if m = Tjk k j n ∧ 1 ≤ n k then G⁻¹ * sn2P u m * (lam j k * φ j (m j)) else 0 := by
    intro m
    unfold closedMigrationRates
    rw [Finset.mul_sum]
    ...
    rw [Finset.mul_sum]
    apply Finset.sum_congr rfl; intro k _
    exact sn2_pre_Tjk φ hφ0 lam hlamdiag (fun m => G⁻¹ * sn2P u m) n m j k
  have hR : HasSum (fun m => (G⁻¹ * ∏ j, (α j ^ m j / phiProd φ j (m j))) * closedMigrationRates lam φ m n)
      (∑ j, ∑ k, if 1 ≤ n k then G⁻¹ * sn2P u (Tjk k j n) * (lam j k * φ j (Tjk k j n j)) else 0) := by
    simp only [hpt]
    exact hasSum_sum (fun j _ => hasSum_sum (fun k _ => sn2_hasSum_single _ _ _))
  rw [hL.tsum_eq, hR.tsum_eq]
  have hper : ∀ k, (∑ j, if 1 ≤ n k then G⁻¹ * sn2P u (Tjk k j n) * (lam j k * φ j (Tjk k j n j)) else 0)
      = G⁻¹ * sn2P u n * ∑ j, lam k j * φ k (n k) := by
    intro k
    by_cases hk : 1 ≤ n k
    · simp only [hk, if_true]
      rw [mul_assoc, sn2_partial_closed u α φ hu lam hlamdiag htraffic.1 htraffic.2.2 n k hk,
        Finset.mul_sum]
      apply Finset.sum_congr rfl; intro j _
      by_cases hjk : j = k
      · subst hjk; simp [hlamdiag]
      · rw [sn2_Tjk_apply_k (Ne.symm hjk)]; ring
    · have : n k = 0 := by omega
      simp [hk, this, hφ0]
  have hR' : (∑ j, ∑ k, if 1 ≤ n k then G⁻¹ * sn2P u (Tjk k j n) * (lam j k * φ j (Tjk k j n j)) else 0)
      = G⁻¹ * sn2P u n * ∑ k, ∑ j, lam k j * φ k (n k) := by
    rw [Finset.sum_comm, Finset.sum_congr rfl (fun k _ => hper k), ← Finset.mul_sum]
  rw [hR']
  rfl
