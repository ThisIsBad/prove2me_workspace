...
theorem solution (C : ℕ) :
    StrictMonoOn (fun ν : ℝ => erlang ν (C + 1)) (Set.Ioi 0)
      ∧ StrictMonoOn (fun ν : ℝ => ν * (1 - erlang ν (C + 1))) (Set.Ioi 0) :=
  ⟨fun a ha _ _ hab => sn3_erlang_strict (C + 1) (by omega) (le_of_lt ha) hab,
    fun a ha _ _ hab => sn3_carried_strict (C + 1) (by omega) (le_of_lt ha) hab⟩
