...
theorem solution (α β h p d : ℝ) (hα0 : 0 < α) (hα1 : α ≤ 1) (hβ0 : 0 < β)
    ...
    ∃ k : ℕ, IsMinOn (meanCost α β h p d) Set.univ (k * d)
      ∧ ∀ S, IsMinOn (meanCost α β h p d) Set.univ S → k * d ≤ S := by
  classical
  have hγ : p / (p + h) < 1 := by rw [div_lt_one (by linarith)]; linarith
  have hex : ∃ n, p / (p + h) ≤ disruptionCdf α β n := by
    have hr0 : 0 ≤ 1 - β := by linarith
    have hr1 : 1 - β < 1 := by linarith
    have ht := (tendsto_pow_atTop_nhds_zero_of_lt_one hr0 hr1).const_mul (α / (α + β))
    rw [mul_zero] at ht
    have hε : 0 < 1 - p / (p + h) := by linarith
    obtain ⟨N, hN⟩ := Filter.eventually_atTop.1 (ht.eventually (gt_mem_nhds hε))
    refine ⟨N, ?_⟩
    rw [sc10_cdf hα0 hα1 hβ0 hβ1]
    have := hN N le_rfl
    linarith
  set k := Nat.find hex with hkdef
  have hk : p / (p + h) ≤ disruptionCdf α β k := Nat.find_spec hex
  have hk' : ∀ n < k, disruptionCdf α β n < p / (p + h) := fun n hn => by
    have := Nat.find_min hex hn
    push Not at this
    exact this
  obtain ⟨h1, h2⟩ := sc10_base_stock α β h p d hα0 hα1 hβ0 hβ1 hh hp hd k hk hk'
  have e : ((k + 1 : ℕ) : ℝ) * d = d + d * k := by push_cast; ring
  refine ⟨k + 1, ?_, ?_⟩
  · rw [e]; exact h1
  · intro S hS; rw [e]; exact h2 S hS
