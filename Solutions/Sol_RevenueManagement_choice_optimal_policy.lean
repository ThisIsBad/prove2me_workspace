...
theorem solution {n : ℕ} (lam : ℕ → ℝ) (P : Finset (Fin n) → Fin n → ℝ)
    ...
    (∀ t t' x, 1 ≤ t → t ≤ t' → t' ≤ T → 1 ≤ x → ∀ S, IsEfficient P p S →
      IsChoiceOptimal lam P p T t x S → ∃ S', IsEfficient P p S' ∧
        IsChoiceOptimal lam P p T t' x S' ∧ purchaseProb P S ≤ purchaseProb P S') := by
  have hopt_iff : ∀ t x S, 0 < lam t → (IsChoiceOptimal lam P p T t x S ↔
      ∀ S'', expRevenue P p S'' - purchaseProb P S'' * choiceDelta lam P p T (t + 1) x ≤
        expRevenue P p S - purchaseProb P S * choiceDelta lam P p T (t + 1) x) := by
    intro t x S hl
    unfold IsChoiceOptimal choiceObj
    exact ⟨fun h S'' => le_of_mul_le_mul_left (h S'') hl,
      fun h S'' => mul_le_mul_of_nonneg_left (h S'') hl.le⟩
  refine ⟨?_, ?_, ?_⟩
  · intro t x ht htT hx
    obtain ⟨y, rfl⟩ : ∃ y, x = y + 1 := ⟨x - 1, by omega⟩
    obtain ⟨S, _, hSe⟩ := Finset.exists_mem_eq_sup' (s := (Finset.univ : Finset (Finset (Fin n))))
      Finset.univ_nonempty (fun S => choiceObj lam P p T t (y + 1) S)
    have hSopt : IsChoiceOptimal lam P p T t (y + 1) S := fun S' => by
      rw [← hSe]
      exact Finset.le_sup' (fun S => choiceObj lam P p T t (y + 1) S) (Finset.mem_univ S')
    refine ⟨S, fun hin => choice_ineff_not_opt lam P p T hP hlam t (y + 1) (by omega)
      (hpos t ht htT) S hin hSopt, hSopt, ?_⟩
    rw [choice_bellman lam P p T t y ht htT, hSe]
  · intro t x x' ht htT hx hxx S _ hSopt
    have hl := hpos t ht htT
    obtain ⟨S', hS'opt, hQ⟩ := choice_argmax_mono P p _ _
      (choiceDelta_conc lam P p T hP hlam (t + 1) x x' hx hxx) S ((hopt_iff t x S hl).1 hSopt)
    have hS'opt' := (hopt_iff t x' S' hl).2 hS'opt
    exact ⟨S', fun hin => choice_ineff_not_opt lam P p T hP hlam t x' (by omega) hl S' hin hS'opt',
      hS'opt', hQ⟩
  · intro t t' x ht htt ht'T hx S _ hSopt
    have hl := hpos t ht (by omega)
    have hl' := hpos t' (by omega) ht'T
    obtain ⟨S', hS'opt, hQ⟩ := choice_argmax_mono P p _ _
      (choiceDelta_time' lam P p T hP hlam (t + 1) (t' + 1) x hx (by omega)) S
      ((hopt_iff t x S hl).1 hSopt)
    have hS'opt' := (hopt_iff t' x S' hl').2 hS'opt
    exact ⟨S', fun hin => choice_ineff_not_opt lam P p T hP hlam t' x hx hl' S' hin hS'opt',
      hS'opt', hQ⟩
