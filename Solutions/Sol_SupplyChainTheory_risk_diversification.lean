...
theorem solution (α β h p d : ℝ) (hα0 : 0 < α) (hα1 : α ≤ 1) (hβ0 : 0 < β)
    ...
    (IsMinOn (meanCost α β h p d) Set.univ S → IsMinOn (meanCost α β h p (N * d)) Set.univ (N * S))
      ...
      ∧ varCost α β h p (N * d) (N * S) = (N : ℝ) ^ 2 * varCost α β h p d S := by
  have hN' : (0 : ℝ) < N := by exact_mod_cast hN
  have hmax : ∀ a : ℝ, max ((N : ℝ) * a) 0 = N * max a 0 := by
    intro a
    rw [mul_max_of_nonneg _ _ hN'.le, mul_zero]
  have hpc : ∀ T n, periodCost h p (N * d) (N * T) n = N * periodCost h p d T n := by
    intro T n
    unfold periodCost
    have e1 : (N : ℝ) * T - (n + 1) * (N * d) = N * (T - (n + 1) * d) := by ring
    have e2 : ((n : ℝ) + 1) * (N * d) - N * T = N * ((n + 1) * d - T) := by ring
    rw [e1, e2, hmax, hmax]
    ring
  have hmean : ∀ T, meanCost α β h p (N * d) (N * T) = N * meanCost α β h p d T := by
    intro T
    unfold meanCost
    simp only [hpc]
    rw [← tsum_mul_left]
    congr 1; ext n; ring
  refine ⟨fun hS => ?_, hmean S, ?_⟩
  · rw [isMinOn_iff] at hS ⊢
    intro T _
    have e : T = N * (T / N) := by field_simp
    rw [e, hmean, hmean]
    exact mul_le_mul_of_nonneg_left (hS _ (Set.mem_univ _)) hN'.le
  · unfold varCost
    rw [hmean]
    simp only [hpc]
    have : ∀ n, disruptionPmf α β n * ((N : ℝ) * periodCost h p d S n) ^ 2
        = (N : ℝ) ^ 2 * (disruptionPmf α β n * (periodCost h p d S n) ^ 2) := fun n => by ring
    simp only [this]
    rw [tsum_mul_left]
    ring
