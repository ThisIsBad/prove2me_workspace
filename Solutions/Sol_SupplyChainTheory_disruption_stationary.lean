...
theorem solution (α β : ℝ) (hα0 : 0 < α) (hα1 : α ≤ 1) (hβ0 : 0 < β) (hβ1 : β ≤ 1) :
    Summable (disruptionPmf α β) ∧ (∑' n, disruptionPmf α β n) = 1
      ∧ IsStationary α β (disruptionPmf α β)
      ∧ ∀ n, disruptionCdf α β n = 1 - α / (α + β) * (1 - β) ^ n := by
  have hs := sc10_pmf_hasSum hα0 hα1 hβ0 hβ1
  have hs1 := sc10_pmf_succ_hasSum hα0 hα1 hβ0 hβ1
  have hab : α + β ≠ 0 := by linarith
  refine ⟨hs.summable, hs.tsum_eq, ⟨?_, ?_, ?_⟩, sc10_cdf hα0 hα1 hβ0 hβ1⟩
  · rw [hs1.tsum_eq]
    simp only [disruptionPmf]
    field_simp
    ring
  · simp only [disruptionPmf, pow_zero, mul_one]
    field_simp
  · intro n hn
    obtain ⟨m, rfl⟩ : ∃ m, n = m + 1 := ⟨n - 1, by omega⟩
    simp only [disruptionPmf, pow_succ]
    ring
