...
theorem solution (lam mu nu : ℝ) (hlam : 0 < lam) (hmu : lam < mu) (hnu : 0 < nu)
  ...
  · have h1 := hα i
    have h2 := hα (i + 1)
    have h3 := hα (i + 2)
    rw [show i + 1 + 1 = i + 2 from rfl] at h2
    rw [show i + 2 + 1 = i + 3 from rfl] at h3
    rw [h1, h2, h3]
    push_cast
    have : (i : ℝ) + 1 ≠ 0 := by positivity
    have : (i : ℝ) + 2 ≠ 0 := by positivity
    have : (i : ℝ) + 3 ≠ 0 := by positivity
    simp only [div_pow]
    field_simp
    ring
  · have hρ : |lam / mu| < 1 := by
      rw [abs_of_pos (by positivity), div_lt_one hmu0]; exact hmu
    have := (Real.hasSum_pow_div_log_of_abs_lt_one hρ).mul_left (nu / lam)
    have key : (fun j : ℕ => α (j + 1))
        = fun j : ℕ => nu / lam * ((lam / mu) ^ (j + 1) / ((j : ℝ) + 1)) := by
      funext j
      rw [hα j]
      have : (j : ℝ) + 1 ≠ 0 := by positivity
      field_simp
    have e : -(nu / lam) * Real.log (1 - lam / mu) = nu / lam * -Real.log (1 - lam / mu) := by ring
    rw [key, e]
    exact this
