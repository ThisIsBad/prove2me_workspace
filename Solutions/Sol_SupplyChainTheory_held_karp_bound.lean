...
import Theorems.Thm_SupplyChainTheory_revised_cost_identities
...
theorem solution {n : ℕ} (c : Fin n → Fin n → ℝ) (hc : IsMetric c) (hn : 3 ≤ n)
    ...
    graphWeight c G + ∑ i, lam i * ((G.degree i : ℝ) - 2) ≤ optTourLength c := by
  obtain ⟨hA, -, hC⟩ := revised_cost_identities c hc hn lam
  obtain ⟨m, rfl⟩ : ∃ m, n = m + 1 := ⟨n - 1, by omega⟩
  have hm : 2 ≤ m := by omega
  -- Compare the optimal revised 1-tree with an optimal tour through `r`, itself a 1-tree.
  obtain ⟨τ, hτr, hτ⟩ := exists_opt_tour_from c r
  have h1 : Is1Tree r (cycG τ) := hτr ▸ cycG_is1Tree hm τ
  have hsymm : ∀ i j, revisedCost c lam i j = revisedCost c lam j i := by
    intro i j; simp only [revisedCost]; rw [hc.symm i j]; ring
  have key : graphWeight (revisedCost c lam) G ≤ optTourLength c + 2 * ∑ i, lam i := by
    calc graphWeight (revisedCost c lam) G ≤ graphWeight (revisedCost c lam) (cycG τ) := hopt _ h1
      _ = tourLength (revisedCost c lam) τ := cycG_weight hm _ hsymm τ
      _ = optTourLength c + 2 * ∑ i, lam i := by rw [hA, hτ]
  rw [hC r G hG] at key
  have hsplit : ∑ i, lam i * ((G.degree i : ℝ) - 2) = ∑ i, (G.degree i : ℝ) * lam i - 2 * ∑ i, lam i := by
