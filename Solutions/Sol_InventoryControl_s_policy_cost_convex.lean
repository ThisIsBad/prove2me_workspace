...
theorem solution (D : DiscreteDemand) (h b1 : ℝ) (hh : 0 < h) (hb : 0 < b1) :
    (∀ k : ℤ, sPolicyCost D h b1 (k + 1) - sPolicyCost D h b1 k
        ≤ sPolicyCost D h b1 (k + 2) - sPolicyCost D h b1 (k + 1))
      ∧ Filter.Tendsto (sPolicyCost D h b1) (Filter.cocompact ℤ) Filter.atTop := by
  refine ⟨fun k => ?_, ?_⟩
  · have := ic5_d_mono D h b1 (by linarith) (show k ≤ k + 1 by omega)
    have e : k + 1 + 1 = k + 2 := by ring
    rw [e] at this; exact this
  · rw [_root_.cocompact_eq_atBot_atTop, Filter.tendsto_sup]
    constructor
    · rw [Filter.tendsto_atTop]
      intro B
      rw [Filter.eventually_atBot]
      refine ⟨min 0 ⌊D.mean - B / b1⌋, fun k hk => ?_⟩
      have hk0 : k ≤ 0 := le_trans hk (min_le_left _ _)
      have hk1 : k ≤ ⌊D.mean - B / b1⌋ := le_trans hk (min_le_right _ _)
      have hk2 : (k : ℝ) ≤ D.mean - B / b1 :=
        le_trans (by exact_mod_cast hk1) (Int.floor_le _)
      have hsum : ∑ j ∈ Finset.Icc (1 : ℤ) k, (j : ℝ) * D.p (k - j).toNat = 0 := by
        rw [Finset.Icc_eq_empty (by omega)]; simp
      unfold sPolicyCost; rw [hsum]
      have hB : b1 * (B / b1) = B := by field_simp
      nlinarith
    · rw [Filter.tendsto_atTop]
      intro B
      have hpos : 0 < h + b1 := by linarith
      have hc1 : (b1 + h / 2) / (h + b1) < 1 := by
        rw [div_lt_one hpos]; linarith
      obtain ⟨N, hN⟩ := ic5_cdf_eventually D hc1
      have hcc : (h + b1) * ((b1 + h / 2) / (h + b1)) = b1 + h / 2 := by field_simp
      have hinc : ∀ k : ℤ, (N : ℤ) ≤ k →
          h / 2 ≤ sPolicyCost D h b1 (k + 1) - sPolicyCost D h b1 k := by
        intro k hk
        rw [ic5_g_succ]
        have := ic5_cdf_mono D hk
        nlinarith
      have hlin : ∀ t : ℕ,
          sPolicyCost D h b1 N + t * (h / 2) ≤ sPolicyCost D h b1 (N + t) := by
        intro t
        induction t with
        | zero => simp
        | succ n ih =>
          have := hinc (N + n) (by omega)
          have e : ((N : ℤ) + ((n + 1 : ℕ) : ℤ)) = N + n + 1 := by push_cast; ring
          rw [e]; push_cast; linarith
      rw [Filter.eventually_atTop]
      refine ⟨(N : ℤ) + max 0 ⌈(B - sPolicyCost D h b1 N) / (h / 2)⌉, fun k hk => ?_⟩
      have hk0 : (N : ℤ) ≤ k := by have := le_max_left 0 ⌈(B - sPolicyCost D h b1 N) / (h / 2)⌉; omega
      have := hlin (k - N).toNat
      have e : (N : ℤ) + (((k - N).toNat : ℕ) : ℤ) = k := by omega
      rw [e] at this
      have ht : ((((k - N).toNat : ℕ) : ℤ) : ℝ) = (k : ℝ) - N := by
        rw [Int.toNat_of_nonneg (by omega)]; push_cast; ring
      have ht' : (((k - N).toNat : ℕ) : ℝ) = (k : ℝ) - N := by exact_mod_cast ht
      have hceil : (B - sPolicyCost D h b1 N) / (h / 2) ≤ (k : ℝ) - N := by
        have h1 := Int.le_ceil ((B - sPolicyCost D h b1 N) / (h / 2))
        have h2 : ⌈(B - sPolicyCost D h b1 N) / (h / 2)⌉ ≤ k - N := by
          have := le_max_right 0 ⌈(B - sPolicyCost D h b1 N) / (h / 2)⌉; omega
        have h3 : ((⌈(B - sPolicyCost D h b1 N) / (h / 2)⌉ : ℤ) : ℝ) ≤ ((k - N : ℤ) : ℝ) := by
          exact_mod_cast h2
        push_cast at h3; linarith
      rw [ht'] at this
      have hh2 : 0 < h / 2 := by linarith
      rw [div_le_iff₀ hh2] at hceil
      linarith
