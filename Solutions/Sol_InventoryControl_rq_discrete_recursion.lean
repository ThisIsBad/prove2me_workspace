...
theorem solution (D : DiscreteDemand) (h b1 A μ : ℝ) (hh : 0 < h) (hb : 0 < b1)
  ...
  have hnext : ic5W D h b1 Rnext (Q + 1)
      = ic5W D h b1 Rstar Q + min (sPolicyCost D h b1 Rstar) (sPolicyCost D h b1 (Rstar + Q + 1)) :=
    ic5_W_next D h b1 Q Rstar
  constructor
  · intro R
    rw [ic5_W_le_iff D h b1 A μ _ _ (Q + 1) (Nat.succ_pos Q), hnext]
    exact ic5_W_lower D h b1 hpos Q hQ Rstar hW R
  · rw [ic5_cost_eq D h b1 A μ _ (Q + 1) (Nat.succ_pos Q), ic5_cost_eq D h b1 A μ _ Q hQ, hnext]
    have hQ' : (Q : ℝ) ≠ 0 := by positivity
